...
func (u *User) CreateSession() (sess Session, err error) {
	...
	err = Db.QueryRow(cmd2, u.ID, u.Email).Scan(&sess.ID, &sess.UUID, &sess.Email, &sess.UserID, &sess.CreatedAt)
	return sess, err
}
...
func (sess *Session) GetUserBySession() (u User, err error) {
	u = User{}
	cmd := `select id,uuid,name,email,created_at from users where id = $1`
	err = Db.QueryRow(cmd, sess.UserID).Scan(&u.ID, &u.UUID, &u.Name, &u.Email, &u.CreatedAt)
	return u, err
}
